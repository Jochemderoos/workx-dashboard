...
export default function VakantiesPage() {
  ...
  // Pending vacation requests state (for admin/partner/office manager)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch pending requests for admins/partners/office managers
      const pendingRes = await fetch('/api/vacation/requests?status=PENDING')
      if (pendingRes.ok) {
  ...
  // Handle approve/reject vacation request
  ...
  return (
    <div className="space-y-8 fade-in relative">
      ...
      {!isAdmin && (() => {
        ...
        const recentRejected = myVacationRequests.filter((req) => req.status === 'REJECTED' && (now - new Date((req as any).updatedAt || (req as any).createdAt).getTime()) < sevenDays)
