...
export default function DashboardHome() {
  ...
  const pendingDeclarations: any[] = summaryData?.pendingDeclarations || []
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">{pendingVacationRequests.length > 0 ? 'Wachtend op goedkeuring' : 'Geen openstaande aanvragen'}</p>
                </div>
              </div>
              <span className={`px-3 py-1 rounded-full ${pendingVacationRequests.length > 0 ? 'bg-orange-500/20 text-orange-300' : 'bg-white/5 text-gray-500'} text-xs font-bold`}>
      ...
      {currentUser?.role === 'ADMIN' && (
        <div className={`relative overflow-hidden rounded-2xl border-2 ${pendingDeclarations.length > 0 ? 'border-green-400 shadow-lg shadow-green-500/20' : 'border-green-500/30'} bg-gradient-to-br ${pendingDeclarations.length > 0 ? 'from-green-500/20 via-green-500/10 to-emerald-500/10' : 'from-green-500/10 via-green-500/5 to-transparent'} p-5`}>
          ...
          {pendingDeclarations.length > 0 && (
            <div className="absolute inset-0 rounded-2xl border-2 border-green-400/30 animate-pulse pointer-events-none" />
          )}
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl ${pendingDeclarations.length > 0 ? 'bg-green-500/30' : 'bg-green-500/20'} flex items-center justify-center`}>
                  <Icons.euro className="text-green-400" size={20} />
                </div>
                <div>
                  <h3 className="font-semibold text-white">Openstaande Declaraties</h3>
                  <p className="text-xs text-gray-400">{pendingDeclarations.length > 0 ? 'Wachtend op betaling' : 'Geen openstaande declaraties'}</p>
                </div>
              </div>
              <span className={`px-3 py-1 rounded-full ${pendingDeclarations.length > 0 ? 'bg-green-500/20 text-green-300' : 'bg-white/5 text-gray-500'} text-xs font-bold`}>
            ...
            {pendingDeclarations.length > 0 ? (
              <div className="space-y-3">
                {pendingDeclarations.map((decl: any) => {
                  const submittedDate = decl.submittedAt ? new Date(decl.submittedAt).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short', year: 'numeric' }) : ''
                  const totalAmount = new Intl.NumberFormat('nl-NL', { style: 'currency', currency: 'EUR' }).format(decl.totalAmount || 0)
                  const itemCount = decl.items?.length || 0

                  return (
                    <div key={decl.id} className="bg-white/[0.05] rounded-xl p-4 border border-green-500/20">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="min-w-0 flex-1">
                          <p className="text-white font-medium text-sm truncate">{decl.employeeName}</p>
                          <p className="text-xs text-gray-500">
                            {submittedDate} · {itemCount} post{itemCount !== 1 ? 'en' : ''}
                            {decl.holdingName && ` · ${decl.holdingName}`}
                          </p>
                        </div>
              ...
              <p className="text-sm text-gray-500 text-center py-2">Geen openstaande declaraties.</p>
            )}
          </div>
        </div>
      )}

      {/* VAKANTIEAANVRAAG STATUS WIDGET - for all users with recently decided requests */}
      {(() => {
        const now = new Date().getTime()
        const sevenDays = 7 * 24 * 60 * 60 * 1000
        const recentApproved = myVacationRequests.filter((req: any) => req.status === 'APPROVED' && (now - new Date(req.updatedAt || req.createdAt).getTime()) < sevenDays)
        const recentRejected = myVacationRequests.filter((req: any) => req.status === 'REJECTED' && (now - new Date(req.updatedAt || req.createdAt).getTime()) < sevenDays)

        return (
          <>
            {recentApproved.length > 0 && (
              <div className="relative overflow-hidden rounded-2xl border border-green-500/30 bg-gradient-to-br from-green-500/15 via-green-500/5 to-transparent p-5">
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-white text-sm">Vakantieaanvraag goedgekeurd</h3>
                        <p className="text-xs text-green-400/80">{recentApproved.length === 1 ? 'Je aanvraag is goedgekeurd!' : `${recentApproved.length} aanvragen goedgekeurd!`}</p>
                      </div>
                    </div>
                    <Link href="/dashboard/vakanties" className="text-xs text-green-400 hover:text-green-300 transition-colors">
                      Bekijken →
                    </Link>
                  </div>
                  <div className="space-y-2">
                    {recentApproved.map((req: any) => {
                      const start = new Date(req.startDate)
                      const end = new Date(req.endDate)
                      const startStr = start.toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })
                      const endStr = end.toLocaleDateString('nl-NL', { day: 'numeric', month: 'short', year: 'numeric' })
                      return (
                        <div key={req.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-green-500/5 border border-green-500/10">
                          <Icons.calendar className="text-green-400/60 flex-shrink-0" size={14} />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-white">{startStr} – {endStr}</p>
                            <p className="text-xs text-gray-500">{req.days} werkdagen{req.reason ? ` · ${req.reason}` : ''}</p>
                          </div>
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-green-500/10 text-green-400">Goedgekeurd</span>
                        </div>
                      )
                    })}
                  </div>
            ...
            {recentRejected.length > 0 && (
              <div className="relative overflow-hidden rounded-2xl border border-red-500/30 bg-gradient-to-br from-red-500/15 via-red-500/5 to-transparent p-5">
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-white text-sm">Vakantieaanvraag afgewezen</h3>
                        <p className="text-xs text-red-400/80">{recentRejected.length === 1 ? 'Je aanvraag is helaas afgewezen' : `${recentRejected.length} aanvragen afgewezen`}</p>
                      </div>
                    </div>
                    <Link href="/dashboard/vakanties" className="text-xs text-red-400 hover:text-red-300 transition-colors">
                      Bekijken →
                    </Link>
                  </div>
                  <div className="space-y-2">
                    {recentRejected.map((req: any) => {
                      const start = new Date(req.startDate)
                      const end = new Date(req.endDate)
                      const startStr = start.toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })
                      const endStr = end.toLocaleDateString('nl-NL', { day: 'numeric', month: 'short', year: 'numeric' })
                      return (
                        <div key={req.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-red-500/5 border border-red-500/10">
                          <Icons.calendar className="text-red-400/60 flex-shrink-0" size={14} />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-white">{startStr} – {endStr}</p>
                            <p className="text-xs text-gray-500">{req.days} werkdagen{req.reason ? ` · ${req.reason}` : ''}</p>
                          </div>
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-red-500/10 text-red-400">Afgewezen</span>
                        </div>
                      )
                    })}
                  </div>
