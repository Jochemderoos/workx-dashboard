...
export default function PartnerJaarplannenPage() {
  ...
  const addEvaluation = async (planId: string) => {
    ...
    try {
      ...
      setMembers(ms => ms.map(m => m.plan?.id === planId
        ? { ...m, plan: { ...m.plan!, evaluations: [ev, ...m.plan!.evaluations] } }
        : m))
  ...
  const stats = useMemo(() => {
    const plans = members.map(m => m.plan).filter(Boolean) as PlanData[]
    ...
    return { totalItems, totalDone, activePlans, totalTeam: members.length }
  }, [members])
      ...
      <div className="rounded-2xl border border-purple-500/20 bg-gradient-to-br from-purple-500/10 via-indigo-500/5 to-transparent p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-6 text-right">
            <div>
              <p className="text-2xl font-bold text-white tabular-nums">{stats.activePlans}<span className="text-sm text-white/40">/{stats.totalTeam}</span></p>
              <p className="text-[10px] uppercase tracking-wider text-white/40">team gestart</p>
            </div>
            <div>
        ...
        <div className="mt-4 pt-4 border-t border-white/10 flex items-center gap-3 flex-wrap">
          <Link
            ...
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-sm text-white/80"
          >
            <Icons.eye size={14} />
            Bekijk medewerker-view (jouw eigen plan)
          </Link>
          <p className="text-xs text-white/40">Zo zien medewerkers het. Categorieën: Juridische theorie, Juridische praktijk, Acquisitie, Intern.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {members.map(({ user: u, plan }) => {
          const photo = getPhotoUrl(u.name, u.avatarUrl)
          const initials = u.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
          const total = plan?.items.length || 0
          const done = plan?.items.filter(i => i.status === 'done').length || 0
          const doing = plan?.items.filter(i => i.status === 'doing').length || 0
          ...
          if (plan) for (const it of plan.items) (itemsByCat[it.category] ||= []).push(it)
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/50">
                    {!hasPlan ? 'Nog geen plan gestart' : `${done}/${total} afgerond · ${doing} mee bezig`}
                  </p>
                </div>
              ...
              {hasPlan ? (
                ...
                <div className="mt-3 text-[10px] text-white/40 italic">
                  Wacht op invulling door medewerker.
                </div>
              )}
                ...
                <div className="mt-4 space-y-3" onClick={(e) => e.stopPropagation()}>
                  {hasPlan ? (
                    ...
                    <div className="space-y-3">
                      <p className="text-xs text-white/50">
                        {u.name.split(' ')[0]} heeft nog geen plan ingevuld voor {year}. Het plan bevat 4 categorieën:
                      </p>
                      <div className="grid grid-cols-2 gap-2">
                        {['theorie', 'praktijk', 'acquisitie', 'intern'].map(cat => (
                          <div key={cat} className="rounded-lg bg-white/[0.03] border border-white/5 p-2">
                            <p className={`text-[10px] uppercase tracking-wider font-semibold ${CAT_COLOR[cat]}`}>{CAT_LABEL[cat]}</p>
                          </div>
                        ))}
                      </div>
                      <p className="text-[11px] text-white/40">Stuur eventueel een herinnering — de pagina staat onder Tools → Mijn Jaarplan.</p>
                    </div>
                  )}
                </div>
              )}
