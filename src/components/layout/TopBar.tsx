...
const navigationItems = [
  ...
  { href: '/dashboard/vakanties', icon: Icons.sun, label: 'Vakanties & Verlof', keywords: ['vakantie', 'vrij', 'verlof', 'afwezig'], roles: ['EMPLOYEE', 'PARTNER', 'ADMIN'] },
  ...
  { href: '/dashboard/partners/sollicitaties', icon: Icons.userPlus, label: 'Sollicitaties', keywords: ['sollicitatie', 'sollicitant', 'cv', 'vacature', 'werving'], roles: ['PARTNER', 'ADMIN'] },
  ...
  { href: '/dashboard/pitch', icon: Icons.file, label: 'Pitch Maker', keywords: ['pitch', 'pdf', 'document', 'cv', 'team'], roles: ['EMPLOYEE', 'PARTNER', 'ADMIN'] },
...
function TopBarComponent({ user }: TopBarProps) {
  ...
  return (
    <header className="h-16 border-b flex items-center justify-between px-4 md:px-8 relative z-[100]" style={{ borderColor: 'var(--color-border-subtle)', background: 'var(--color-bg-topbar)' }}>
      ...
      {/* Center: flex-spacer (oude search verwijderd — alle search via
          de prominente home-zoekbalk en Ctrl/⌘+K palette) */}
      <div className="flex-1" />
      {/* (oude search-code is volledig verwijderd) */}
